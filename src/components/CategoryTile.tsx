...
export function CategoryTile({ category, priority = false }: CategoryTileProps) {
  return (
    ...
    >
      ...
      <span className="border-t border-brand-100 px-2 py-2.5 text-center text-sm leading-snug font-semibold text-brand-900 transition-colors group-hover:text-brand-600">
        {category.name}
      </span>
    </Link>
  );
...
export function CategoryGrid({
  ...
  return (
    <div
      className={`grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6 ${className}`}
    >
