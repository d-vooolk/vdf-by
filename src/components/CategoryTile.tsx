...
export function CategoryTile({ category, priority = false }: CategoryTileProps) {
  return (
    ...
    >
      ...
      <span className="border-t border-brand-100 px-2 py-2 text-center text-[13px] leading-snug font-semibold sm:text-sm text-brand-900 transition-colors group-hover:text-brand-600">
        {category.name}
      </span>
    </Link>
  );
