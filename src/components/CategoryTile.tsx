...
export function CategoryTile({ category, priority = false }: CategoryTileProps) {
  return (
    ...
    >
      ...
      <span className="border-t border-brand-100 px-1.5 py-2 text-center text-xs leading-snug sm:px-2 sm:py-2.5 sm:text-sm font-semibold text-brand-900 transition-colors group-hover:text-brand-600">
        {category.name}
      </span>
    </Link>
  );
...
export function CategoryGrid({
  ...
  return (
    <div
      className={`grid grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-3 lg:grid-cols-6 ${className}`}
    >
